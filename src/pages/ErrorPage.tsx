  ...
  return (
    <section className="section flex min-h-[60vh] flex-col items-center justify-center py-14 md:py-20">
      ...
      <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }} className="text-center">
        <div className="mb-3 inline-flex items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-3 py-1 text-xs tracking-wide text-cyan-300">
          Error
          <span className="rounded bg-white/10 px-1.5 py-0.5 text-white/90">{code}</span>
        </div>
        <h1 className="text-3xl font-semibold text-white/90 md:text-4xl">{info.title}</h1>
        <p className="mt-2 max-w-xl text-balance text-sm text-gray-300 md:text-base">{info.description}</p>

        <div className="mt-6 flex flex-wrap items-center justify-center gap-3 text-sm">
          <Link to="/" className="rounded-md border border-white/10 bg-white/5 px-4 py-2 text-white/90 hover:bg-white/10">Go to homepage</Link>
          {code !== '404' && (
            <Link to="/contact" className="rounded-md border border-white/10 bg-white/5 px-4 py-2 text-white/90 hover:bg-white/10">Contact</Link>
          )}
        </div>
      </motion.div>
    </section>
